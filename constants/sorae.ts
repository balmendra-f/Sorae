...
export const soraeRitualPrompts: Record<
  RitualId,
  Record<SoraeLocale, string>
> = {
  "clear-thought": {
...
export const getSoraeRitualPrompt = (
  ritualId: RitualId,
  locale: SoraeLocale,
) => soraeRitualPrompts[ritualId][locale];

export const localizeSoraeRitualPrompt = (
  text: string,
  locale: SoraeLocale,
) => {
  const trimmedText = text.trim();
  const matchingPrompt = Object.values(soraeRitualPrompts).find((prompts) =>
    Object.values(prompts).includes(trimmedText),
  );

  return matchingPrompt ? matchingPrompt[locale] : text;
};
